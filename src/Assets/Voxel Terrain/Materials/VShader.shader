Shader "Custom/VShader" {
	...
	SubShader {
		...
		uniform sampler2D _TopTex, _SideTex;
		
		// texture uv (for TRANSFORM_TEX)
		uniform float4 _TopTex_ST,  _SideTex_ST;
		...
		void vert (inout appdata_full v, out Input o) {
			...
			// uv projection plan
			//o.uv_MainTex = TRANSFORM_TEX(v.texcoord, _TopTex);
			//float2 planX = float2(v.texcoord.y, v.texcoord1.y);
			//float2 planY = float2(v.texcoord.xy);
			//float2 planZ = float2(v.texcoord1.xy);
			float2 planX = float2(v.vertex.y, v.vertex.z);
			float2 planY = float2(v.vertex.x, v.vertex.z);
			float2 planZ = float2(v.vertex.x, v.vertex.y);
			
			// apply uv plan on uv texture
			o.uv_x = o.pos.yz;//planX;//TRANSFORM_TEX(planX, _TopTex);
			o.uv_y = o.pos.xz;//planY;//TRANSFORM_TEX(planY, _SideTex);
			o.uv_z = o.pos.xy;//planZ;//TRANSFORM_TEX(planZ, _SideTex);
			
			// variable for test normal
		...
		void surf (Input IN, inout SurfaceOutput o) {
			o.Albedo =	tex2D(_SideTex, IN.uv_x).rgb * float3(IN.alpha.x, IN.alpha.x, IN.alpha.x) +
						tex2D(_TopTex,  IN.uv_y).rgb * float3(IN.alpha.y, IN.alpha.y, IN.alpha.y) +
						tex2D(_SideTex, IN.uv_z).rgb * float3(IN.alpha.z, IN.alpha.z, IN.alpha.z);
		}
